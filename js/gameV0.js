...
function create() {
    ...
    quizzText = this.add.text(200,135, "Test de Personnalité",{ fontFamily: 'carterone', fontSize: 20, color: '#000000' });
    welcomeText = this.add.text(120,230, "Play Start to play the game!!",{ fontFamily: 'skranji', fontSize: 18, color: '#000000' });
    ...
    QuestionText = this.add.text(95, 60, quizz.questions[0].title,{ fontFamily: 'carterone', fontSize: 17, color: '#00ff00' });
    ...
    for(let i=0; i<quizz.questions[0].answers.length; i++) {
        answerText[i] = this.add.text(110, 210 + i*110, quizz.questions[0].answers[i], { fontFamily: 'Arial', fontSize: 16, color: '#000000'});
        //answerImage[i].setOrigin(0,0);
...
function checkAnswer(Answer_index){
    ...
    if (Answer_index==0) resultOK += plusPointB
    else if (Answer_index==1) resultOK += plusPointA
    else if (Answer_index ==2) resultOK += plusPointC
...
function displayGameOver() {
    ...
    if (resultOK>15) welcomeText.text = "15 points ou plus " + resultOK + "\nVotre score indique que vous savez très bien gérer votre stress. Vousêtes très probablement perçu(e) comme quelqu’un de très détendupresque toujours capable de garder le sens des proportions.Cependant, cette prédisposition tout à fait appréciable ne doit pasvous empêcher de vous préparer aux situations stressantes qui nemanqueront pas de se présenter. Vous devriez arriver à gérer ces fac-teurs de stress imprévus en les anticipant.N’oubliez pas non plus que le stress peut être positif puisqu’il nouspousse à réagir et à nous dépasser." ;    
    else if (resultOK>10) welcomeText.text = "entre 10 et 15 points " + resultOK ;    
    else welcomeText.text = "moin de 10 " + resultOK + "/10\n Presser le bouton pour recommencer";    
